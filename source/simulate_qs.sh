...
## @brief This is an automatic scripts for running questasim and recording the time and memory used.
...
# @ToDo: When the target result vcd file is very big, it will prominently reduce the performance of questasim.
# @ToDo: Maybe you should do not ask it to generate vcd file to remove the influence from disk performance and different result file size.
##  END
...
compile_command="vlog ./testbench.v ./module.v"
run_command="vsim -c -voptargs=+acc work.top_tb -do ./run.do"
...
execute_command() {
    ...
    TEMP_FILE=$(mktemp)
    cp ../inputs/prototypes/run.do ../outputs/temp
    cd ../outputs/temp || exit
    vlib work
   ...
   # 提取并显示感兴趣的信息
    ...
    # 提取感兴趣的信息并保存到JSON文件
    simulate_time=$(grep 'Elapsed time' "$TEMP_FILE" | awk '{print $NF}')
    simulate_memory=$(grep 'Maximum resident set size' "$TEMP_FILE" | awk '{print $NF}')

# you may find that the same command is called twice, it is necessary because if you call it by /usr/bin/time vsim
# will not generate right vcd file.
# Besides, if you do not call it by time you will not have the information about memory used.
#    $run_command &> "$TEMP_FILE"


    # 提取并显示感兴趣的信息
    echo "Summary for command: run"
    grep 'Elapsed time' "$TEMP_FILE"
    echo 'Maximum resident set size' "$simulate_memory"
